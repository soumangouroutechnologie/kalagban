...
export default function SellerKycModal({
  ...
  useEffect(() => {
    if (step === 3 && canvasRef.current && !signatureData && !isAlreadyCertified) {
  ...
  }, [step, signatureData, isAlreadyCertified]);

  if (!isOpen) return null;

  const startDrawing = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    if (isAlreadyCertified) return;
    setIsDrawing(true);
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    if (!isDrawing || isAlreadyCertified) return;
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
  ...
  const handleStorePhotosSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length > 0) {
      const newFiles = [...storePhotos, ...validFiles].slice(0, 4);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/75 backdrop-blur-md animate-in fade-in duration-200">
      ...
      {/* Strict Print CSS to isolate ONLY the official printable document */}
      <style jsx global>{`
        @media print {
          body * {
            visibility: hidden !important;
          }
          #printable-kyc-dossier, #printable-kyc-dossier * {
            visibility: visible !important;
          }
          #printable-kyc-dossier {
      ...
      <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[92vh] overflow-y-auto shadow-2xl border border-gray-100 relative flex flex-col custom-scrollbar">
        ...
        <div className="sticky top-0 bg-white/95 backdrop-blur-md z-30 px-6 sm:px-8 py-5 border-b border-gray-100 flex items-center justify-between no-print">
          <div className="flex items-center gap-3.5">
            <div className={`w-11 h-11 rounded-2xl flex items-center justify-center font-black shadow-inner ${
              isAlreadyCertified ? "bg-emerald-50 text-emerald-600" : "bg-indigo-50 text-indigo-600"
            }`}>
              <ShieldCheck size={26} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full ${
                  isAlreadyCertified ? "bg-emerald-100 text-emerald-800" : "bg-indigo-50 text-indigo-600"
                }`}>
                  {isAlreadyCertified ? "Boutique Certifiée 🛡️" : "Dossier Officiel"}
                </span>
                {isAlreadyCertified ? (
                  <span className="text-xs font-bold text-emerald-700 flex items-center gap-1">
                    <Lock size={12} /> Dossier Validé &amp; Verrouillé
                  </span>
                ) : (
              ...
              <h3 className="text-xl font-black text-gray-900 tracking-tight">
                {isAlreadyCertified ? "Certificat & Dossier KYC de la Boutique" : "Certification de la Boutique"}
              </h3>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {isAlreadyCertified && (
              <button
                type="button"
                onClick={handlePrint}
                className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-extrabold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
        ...
        {/* ========================================================================= */}
        {/* MODE 1: ALREADY CERTIFIED (READ-ONLY & CLEAN PRINTABLE DOSSIER) */}
        {/* ========================================================================= */}
        {isAlreadyCertified && existingKyc ? (
          <div id="printable-kyc-dossier" className="p-6 sm:p-8 space-y-6">
            
            {/* Official Header for Printable PDF */}
            <div className="border-b-2 border-emerald-600 pb-4 mb-4 flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl font-black text-emerald-950 tracking-tight">KALAGBAN MARKETPLACE</h1>
                ...
                <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mt-0.5">
                  Direction de la Conformité &amp; Sécurité Marchande
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs font-black text-gray-900">CERTIFICAT D&apos;ENREGISTREMENT KYC</p>
                <p className="text-[10px] text-gray-400 font-mono">ID: {existingKyc.id?.slice(0, 13).toUpperCase() || "KLG-CERT"}</p>
                {existingKyc.reviewed_at && (
                  <p className="text-[10px] text-emerald-700 font-bold">Validé le : {new Date(existingKyc.reviewed_at).toLocaleDateString('fr-FR')}</p>
                )}
              </div>
            </div>

            {/* Certified Status Banner */}
            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4.5 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-black text-sm text-emerald-950">
                    Boutique Officiellement Certifiée &amp; Conforme
                  </h4>
                  <p className="text-xs text-emerald-800 font-medium">
                    Ce dossier a été validé par les auditeurs de Kalagban. Les données et pièces justificatives sont archivées et verrouillées.
                  </p>
                </div>
              </div>
            </div>

            {/* Section 1: Coordonnées & Gérant */}
            <div className="bg-slate-50/80 p-5 rounded-2xl border border-gray-200/80 space-y-4">
              <h4 className="text-xs font-black text-gray-900 uppercase tracking-wider border-b border-gray-200 pb-2 flex items-center justify-between">
                <span>1. Informations Générales &amp; Gérant</span>
                <span className="text-[10px] text-gray-400 font-bold">Boutique : {shopName}</span>
              </h4>

              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-xs">
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase">Nom du Responsable</span>
                  <p className="font-black text-gray-900 text-sm mt-0.5">{existingKyc.seller_name}</p>
                </div>

                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase">Type &amp; N° Pièce</span>
                  <p className="font-extrabold text-gray-900 mt-0.5 uppercase">
                    {existingKyc.id_type} : <span className="font-mono text-indigo-700">{existingKyc.id_number}</span>
                  </p>
                </div>

                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase">WhatsApp Principal</span>
                  <p className="font-black text-emerald-700 mt-0.5">{existingKyc.primary_phone}</p>
                </div>

                {existingKyc.secondary_phone && (
                  <div>
                    <span className="text-[10px] font-bold text-gray-400 uppercase">Numéro Secondaire</span>
                    <p className="font-bold text-gray-700 mt-0.5">{existingKyc.secondary_phone}</p>
                  </div>
                )}

                <div className="col-span-2">
                  <span className="text-[10px] font-bold text-gray-400 uppercase">Adresse Physique</span>
                  <p className="font-bold text-gray-800 mt-0.5">{existingKyc.store_address}</p>
                </div>
              </div>

              {existingKyc.location_description && (
                <div className="pt-2 border-t border-gray-200/60 text-xs">
                  <span className="text-[10px] font-bold text-gray-400 uppercase">Repères d&apos;accès</span>
                  <p className="font-medium text-gray-600 mt-0.5">{existingKyc.location_description}</p>
                </div>
              )}
            </div>

            {/* Section 2: Photos d'Identité & Pièces Justificatives */}
            <div className="space-y-3">
              <h4 className="text-xs font-black text-gray-900 uppercase tracking-wider">
                2. Pièces Justificatives Archivées
              </h4>

              <div className="grid grid-cols-3 gap-4">
                {/* Photo Portrait */}
                <div className="bg-white rounded-2xl border border-gray-200 p-2.5 text-center shadow-2xs">
                  <span className="text-[10px] font-bold text-gray-500 uppercase block mb-1.5">Photo Gérant</span>
                  <img
                    src={existingKyc.seller_photo_url}
                    alt="Gérant"
                    className="w-full h-36 object-cover rounded-xl border border-gray-100"
                  />
                  <a
                    href={existingKyc.seller_photo_url}
                    target="_blank"
                    rel="noreferrer"
                    className="no-print mt-1.5 text-[10px] font-bold text-indigo-600 hover:underline inline-flex items-center gap-1"
                  >
                    <ExternalLink size={10} /> Agrandir
                  </a>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-2.5 text-center shadow-2xs">
                  ...
                  {existingKyc.id_card_front_url?.endsWith(".pdf") ? (
                    <div className="w-full h-36 rounded-xl bg-indigo-50/50 flex flex-col items-center justify-center text-indigo-600 gap-1">
                      <FileText size={36} />
                      <span className="text-[11px] font-bold">Document PDF</span>
                    </div>
                  ) : (
                    <img
                      src={existingKyc.id_card_front_url}
                      alt="ID Recto"
                      className="w-full h-36 object-cover rounded-xl border border-gray-100"
                    />
                  )}
                  <a
                    href={existingKyc.id_card_front_url}
                    target="_blank"
                    rel="noreferrer"
                    className="no-print mt-1.5 text-[10px] font-bold text-indigo-600 hover:underline inline-flex items-center gap-1"
                  >
                ...
                {/* CNI Verso */}
                {existingKyc.id_card_back_url && (
                  <div className="bg-white rounded-2xl border border-gray-200 p-2.5 text-center shadow-2xs">
                    <span className="text-[10px] font-bold text-gray-500 uppercase block mb-1.5">Pièce Verso</span>
                    {existingKyc.id_card_back_url.endsWith(".pdf") ? (
                      <div className="w-full h-36 rounded-xl bg-indigo-50/50 flex flex-col items-center justify-center text-indigo-600 gap-1">
                        <FileText size={36} />
                        <span className="text-[11px] font-bold">PDF Verso</span>
                      </div>
                    ) : (
                      <img
                        src={existingKyc.id_card_back_url}
                        alt="ID Verso"
                        className="w-full h-36 object-cover rounded-xl border border-gray-100"
                      />
                    )}
                    <a
                      href={existingKyc.id_card_back_url}
                      target="_blank"
                      rel="noreferrer"
                      className="no-print mt-1.5 text-[10px] font-bold text-indigo-600 hover:underline inline-flex items-center gap-1"
                    >
            ...
            {/* Section 3: Photos Boutique Physique */}
            {existingKyc.store_photos && existingKyc.store_photos.length > 0 && (
              <div className="space-y-3">
                <h4 className="text-xs font-black text-gray-900 uppercase tracking-wider">
                  3. Localisation &amp; Visuels Boutique Physique
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {existingKyc.store_photos.map((url: string, idx: number) => (
                    <div key={idx} className="rounded-xl border border-gray-200 p-1.5 bg-white shadow-2xs">
                      <img src={url} alt={`Magasin ${idx + 1}`} className="w-full h-20 object-cover rounded-lg" />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Section 4: Signature & Sceau Numérique */}
            <div className="bg-slate-50/80 p-5 rounded-2xl border border-gray-200/80 flex items-center justify-between gap-6">
              <div>
                <h4 className="text-xs font-black text-gray-900 uppercase tracking-wider">
                  4. Signature Manuscrite &amp; Engagement
                </h4>
                <p className="text-[11px] text-gray-500 font-medium mt-1">
                  Certifié sur l&apos;honneur par <strong>{existingKyc.seller_name}</strong>
                </p>
                <span className="text-[10px] font-bold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded-md inline-block mt-2">
                  ✓ Charte de conformité Kalagban signée et approuvée
                </span>
              </div>

              <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-inner text-center">
                <img
                  src={existingKyc.signature_url}
                  alt="Signature"
                  className="h-14 max-w-45 object-contain mx-auto"
                />
                <span className="text-[8px] text-gray-400 font-mono block mt-1">Signature Numérique Certifiée</span>
              </div>
            </div>

            {/* Footer Close in Modal */}
            <div className="no-print pt-4 border-t border-gray-100 flex items-center justify-between">
              <span className="text-xs font-bold text-gray-400 flex items-center gap-1">
                <Lock size={14} /> Fiche officielle en lecture seule
              </span>
              <button
                type="button"
                onClick={onClose}
                className="px-6 py-2.5 rounded-xl bg-gray-900 hover:bg-gray-800 text-white font-extrabold text-xs transition-colors"
              >
                Fermer
              </button>
            </div>
          ...
          /* ========================================================================= */
          /* MODE 2: SUBMISSION FORM (3 STEPS) FOR PENDING / UNVERIFIED SELLERS */
          /* ========================================================================= */
          <>
            {/* Step Progression Bar */}
            <div className="grid grid-cols-3 border-b border-gray-100 bg-slate-50/50 no-print">
            ...
            {/* Footer Navigation */}
            <div className="sticky bottom-0 bg-white/95 backdrop-blur-md px-6 sm:px-8 py-4 border-t border-gray-100 flex items-center justify-between no-print">
              {step > 1 ? (
                <button
                  type="button"
      ...
      {showConfirmPopup && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-in fade-in duration-150 no-print">
