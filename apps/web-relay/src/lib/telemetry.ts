...
export function getDeviceId(): string {
  ...
  try {
    ...
    if (!id) {
      id = "relay_web_" + Math.random().toString(36).substring(2, 10) + Date.now().toString(36);
      localStorage.setItem(key, id);
    }
    return id;
  } catch {
    return "anonymous_relay_web";
  }
}
...
export async function reportError(params: TelemetryReportParams): Promise<void> {
  ...
  const errorSignature = [params.app || "web-relay", params.message, params.route_path || window.location.pathname].join("::");
...
export function initGlobalTelemetry(appName: TelemetryReportParams["app"] = "web-relay") {
