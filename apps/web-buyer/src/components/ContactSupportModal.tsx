...
export default function ContactSupportModal({
  ...
  // Kalagban Official Emergency WhatsApp
  const kalagbanSupportPhone = "2250777620864";
  const whatsappUrl = `https://wa.me/${kalagbanSupportPhone}?text=${encodeURIComponent(
    `Bonjour Support Kalagban, je souhaite une assistance concernant ${orderCode ? `la commande #${orderCode}` : "mon compte"}.`
  )}`;
  ...
  useEffect(() => {
    ...
    const loadUserProfile = async () => {
      ...
      if (session) {
        ...
        if (profile) {
          setUserName(`${profile.first_name || ""} ${profile.last_name || ""}`.trim() || "Client Kalagban");
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    try {
      const randomCode = `TK-${Math.floor(100000 + Math.random() * 900000)}`;
      ...
      const { data: newTicket, error } = await supabase
        ...
        .insert({
          ...
          user_type: userType,
          user_name: userName.trim() || (userType === "buyer" ? "Client Kalagban" : "Boutique Partenaire"),
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setReplyText("");
      toast.success("Message envoyé à l'équipe support.");
    } catch (err: unknown) {
      ...
      <div className="bg-white rounded-3xl max-w-lg sm:max-w-xl w-full max-h-[92vh] sm:max-h-[90vh] overflow-hidden shadow-2xl border border-gray-100 flex flex-col my-auto">
        ...
        <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black">Support &amp; Assistance Kalagban</h3>
              <p className="text-xs text-gray-400 font-medium">Réponse rapide de nos conseillers 7j/7</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            ...
            <form onSubmit={handleCreateTicket} className="space-y-4">
              ...
              <div className="bg-emerald-50 border border-emerald-200/80 rounded-2xl p-4 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black text-emerald-950">Besoin d&apos;une aide urgente ?</p>
                    <p className="text-[11px] text-emerald-800 font-medium">Échangez directement avec un agent sur WhatsApp</p>
                  </div>
                </div>
                <a
                  href={whatsappUrl}
              ...
              <div>
                ...
                >
                  ...
                  <option value="delivery">🚚 Livraison &amp; Point Relais</option>
                  <option value="payment">💳 Paiement &amp; Remboursement</option>
                  <option value="counterfeit_report">🛡️ Signalement / Non-conformité</option>
                  <option value="technical">⚙️ Problème technique / Compte</option>
                  <option value="other">💬 Autre demande</option>
                </select>
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">Numéro de commande (optionnel)</label>
                <input
                  type="text"
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Ex: Mon colis n'est pas encore arrivé au point relais"
                  value={subject}
              ...
              <div>
                ...
                <textarea
                  required
                  rows={4}
                  placeholder="Expliquez clairement votre situation pour que nous puissions vous aider rapidement..."
                  value={message}
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <Send size={16} />
                    Envoyer ma demande d&apos;assistance
                  </>
                )}
              </button>
            </form>
                  ...
                  >
                    ...
                    >
                      <p className="font-bold text-[10px] opacity-75 mb-1">
                        {msg.sender_type === "admin" ? "🛡️ Support Kalagban" : "👤 Vous"}
                      </p>
            ...
            <div className="space-y-3">
              {loadingTickets ? (
                ...
                <div className="py-12 text-center text-gray-400">
                  ...
                  <p className="font-bold text-gray-700 text-sm">Aucune demande en cours</p>
                  <p className="text-xs mt-1">Cliquez sur &quot;Nouveau Ticket&quot; pour nous envoyer une demande.</p>
                </div>
              ) : (
                tickets.map((t) => (
                  <div
