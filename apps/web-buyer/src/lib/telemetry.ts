...
/**
 * Get or create a persistent anonymous device identifier for telemetry.
 */
export function getDeviceId(): string {
  ...
  try {
    ...
    if (!id) {
      id = "web_" + Math.random().toString(36).substring(2, 10) + Date.now().toString(36);
...
/**
 * Detect client OS and Browser model
 */
...
/**
 * Send an error or telemetry event to the centralized bug tracker.
 */
export async function reportError(params: TelemetryReportParams): Promise<void> {
  if (typeof window === "undefined") return;

  const errorSignature = [params.app || "web-buyer", params.message, params.route_path || window.location.pathname].join("::");
  if (RECENT_ERRORS.has(errorSignature)) return; // Debounce duplicate bursts
...
/**
 * Initializes automatic global crash interception (window.onerror & unhandledrejection)
 */
export function initGlobalTelemetry(appName: TelemetryReportParams["app"] = "web-buyer") {
